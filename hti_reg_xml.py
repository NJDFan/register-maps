...
def detab(text):
    """
    Clean up tabs in triple-quoted strings.
    
    This function removes any consistent whitespace in a text string
    containing newlines.  It will also remove one newline, if present
    from both the beginning or end of a string.
    """
    
    dedented = textwrap.dedent(text)
    start = 1 if (dedented[0] == "\n") else 0
    end = -1 if (dedented[-1] == "\n") else None
    return dedented[start:end]
...
class MissingRequiredAttributeError(_Error):
    """A required attribute for a given element is missing in the XML."""
    pass
    
class MissingComponentError(_Error):
    """The Component called out by the extern attribute of an Instance
    cannot be found."""
    pass
    
class InvalidStructureError(_Error):
    """The XML contained inside an element has a structural error."""
    pass
    
class InvalidValueError(_Error):
    """The XML contained inside an element has an invalid value."""
    pass
...
def _prettyError(fn):
    """A decorator that wraps any exceptions a class member function
    may raise with information about where in the XML the error ocurred.
    """
    
    def _intl(self, *args, **kwargs):
        try:
            return fn(self, *args, **kwargs)
        except Exception as e:
            # Append the location information into the error information.
            try:
                s = "Exception in {cl} {na}, sourced from {fi}:{ln}\n".format(
